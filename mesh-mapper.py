...
# Add margin to filterToggle when collapsed
    ...
    /* USB status box styling - purple border and black background */
    #serialStatus {
      background-color: black !important;
    }
    #serialStatus:parent, div:has(> #serialStatus) {
    ...
    /* Forcefully disable any tooltips on no-GPS drones */
    #activePlaceholder .drone-item.no-gps:hover::after,
    #activePlaceholder .drone-item.no-gps:hover::before {
    ...
    .drone-item.recent {
      box-shadow: 0 0 0 1px lime !important;
    }
    /* Remove green box-shadow from no-GPS drones, keep their blue border */
    .drone-item.recent.no-gps {
      box-shadow: none !important;
    }
    /* DEBUG: Test rule to ensure CSS is working - all drone items should have red background */
    /*
    .drone-item {
    ...
    */
    ...
    /* Serial status box styling - purple border and black background */
    #serialStatus {
      background-color: black !important;
    ...
    /* Target the parent div containing serialStatus for purple border */
    div:has(> #serialStatus) {
      border: 1px solid #FF00FF !important;
      background-color: black !important;
...
<div id="filterBox">
  ...
  <div id="filterContent">
    ...
    <div style="margin-top:8px; width:fit-content; max-width:calc(100% - 16px); margin:8px auto 0 auto; border: 1px solid #FF00FF; background-color: black; padding:2px 8px; display:flex; justify-content:center; align-items:center;">
...
function updateComboList(data) {
  ...
  persistentMACs.forEach(mac => {
    ...
    // Debug logging
    console.log(`MAC: ${mac}, hasGps: ${hasGps}, hasRecentTransmission: ${hasRecentTransmission}, drone_lat: ${det?.drone_lat}, drone_long: ${det?.drone_long}, last_update: ${det?.last_update}, currentTime: ${currentTime}`);
    ...
    if (!hasGps && hasRecentTransmission) {
      ...
      console.log(`Adding no-gps class to ${mac}`);
    } else {
      item.classList.remove('no-gps');
    }
    
    // Mark items seen in the last 5 seconds
    const isRecent = det && ((currentTime - det.last_update) <= 5);
    console.log(`MAC: ${mac}, isRecent: ${isRecent}, applying recent class: ${isRecent}`);
